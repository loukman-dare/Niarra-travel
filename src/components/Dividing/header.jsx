...
import { faArrowDown, faBars, faBurger, faChevronDown, faChevronUp, faDollar, faEur, faGbp, faHamburger, faLocation, faPoundSign, faTimes, faUsd } from '@fortawesome/free-solid-svg-icons'
...
import Enquire from '../Utils/Enquire'
...
const Header = () => {
  ...
  return (
    <div className='flex items-center justify-between py-8 px-9 md:px-24 relative gap-8' >
        <div className='cursor-pointer relative'>
            <svg  className={`${dropOpen || isOpen ?"d" : "logo"}`} width="77" height="24" viewBox="0 0 66 16" xmlns="http://www.w3.org/2000/svg">
        ...
        <DropDown 
            ...
            locations={dropName === "dest" ?["tokyo , JApan","Cairo , Egypt","Paris , France","Rozario , Argentina"]:
                    dropName ==="style" ? ["tokyo , JApan","Cairo , Egypt","Paris , France","Rozario , Argentina","syria , Aleppo","Dubai,UAE" , "Kingston,POI" , "Miami , USA"] :
                    dropName==="about" ? ["tokyo , JApan","Cairo , Egypt","Paris , France","Rozario , Argentina","Dubai,UAE" , "Kingston,POI" , "Miami , USA"] :
                    ["tokyo , JApan","Cairo , Egypt","Paris , France","Rozario , Argentina"]    
                    }
        />
        
        <TinyDropDown isOpen={isOpen} setIsOpen={setIsOpen}/>
        {/* <Enquire /> */}
       
        <div className={`items-center gap-10 relative flex`}>
                <div className={`absolute -left-3/4 ${isOpen ? "block"  : "hidden"} lg:block `}>
                    {moneyDrop && 
                    ...
                    <div className='flex flex-col gap-3 px-5 py-3 bg-white_color absolute'>
                        <div className='flex items-center gap-1 cursor-pointer ' onClick={()=>setMoneyDrop(false)}>
                            <FontAwesomeIcon className={`text-sm font-bold text-gray-500 transform  transition-colors duration-1000 ease-in-out`} 
                            icon={moneyChooce === "USD" ? faUsd : moneyChooce==="EUR" ? faEur : moneyChooce ==="GPB"?faGbp : moneyChooce === "AUD" ? faPoundSign : faDollar} />
                            <a  className={`  text-gray-500  text-xs font-medium transform  transition-colors duration-1000 ease-in-out `}> {moneyChooce}</a>
                        ...
                        </div>
                        {
                            ...
                            .map((coin) => (
                                <motion.div initial="initial" whileHover="hover" key={coin} value={coin} className='flex  items-center gap-1 cursor-pointer' onClick={()=>handleChooseMoney(coin)}>
                    ...
                    <div className={`${moneyDrop ? "hidden" : ""} flex items-center gap-1 cursor-pointer`} onClick={()=>setMoneyDrop(true)}>
                        <FontAwesomeIcon className={`text-sm font-bold mr-1 ${dropOpen || isOpen?'text-black_color' : 'text-white_color'} transform  transition-colors duration-1000 ease-in-out`} 
                        icon={moneyChooce === "USD" ? faUsd : moneyChooce==="EUR" ? faEur : moneyChooce ==="GPB"?faGbp : moneyChooce === "AUD" ? faPoundSign : faDollar} />

                                
                        <a  className={` ${dropOpen || isOpen ? "text-black text-xs font-medium transform  transition-colors duration-1000 ease-in-out" : "text-white text-xs font-medium transform  transition-colors duration-1000 ease-in-out"}} `}> {moneyChooce}</a>
                        <FontAwesomeIcon className={`text-[8px] font-medium ${dropOpen || isOpen? "text-black_color" : "text-white_color"} transform  transition-colors duration-1000 ease-in-out`} icon={faChevronDown} />
                    </div>
                </div>
                <div className='hidden lg:flex outline-none items-center gap-2 py-4 px-6 rounded-full cursor-pointer bg-black_color text-white_color transition-colors duration-700 hover:bg-orange_color'>
        ...
        <div className='flex lg:hidden gap-4 text-white items-center '>
            <div className='cursor-pointer text-xs md:text-lg'> 
                ENIQUIRE
            </div>
            <div
        className={`border cursor-pointer rounded-full w-8 h-8 md:w-10 md:h-10 flex items-center justify-center border-dashed text-xs md:text-lg ${isOpen?"border-black" : "border-white"}`}
      ...
      >
        ...
        >
          <motion.span
            className={`w-5 h-[0.5px] ${isOpen?"bg-black" : "bg-white"} `}
            variants={{
              open: { rotate: 45, y: 3 },
              closed: { rotate: 0, y: 0 },
            }}
          />
          <motion.span
            className='w-5 h-[0.5px] bg-white'
            variants={{
              open: { opacity: 0 },
              closed: { opacity: 1 },
            }}
